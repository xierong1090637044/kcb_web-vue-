<template>
  <div>
    ...
    <div style="background: #FFFFFF;padding: 1.25rem 1.25rem 0;">
      <div class="orderTitle">出库单</div>
      <div class="display_flex" style="margin-bottom: 1.875rem;">
        <div class="tableEle">
          <div class="tableRightEle">出库类型<span style="color: #f30;">*</span></div>
        ...
        <div class="tableEle">
          <div class="tableRightEle">出库日期</div>
        ...
        <template slot-scope="{ row, index }" slot="reserve">
          <div v-if="row.selected_model">
            <div v-for="(model,key) in row.selected_model" :key="key" class="display_flex" style="margin:0.25rem 0.375rem;">
              ...
              <InputNumber placeholder="请输入" size="small" @on-focus="selectIndex = index" v-if="row.goodsName" :value="model.num"
                :min="0" @on-change="handleModelNumChange($event, index,key,model)"></InputNumber>
            </div>
          </div>
          <div v-else>
            <InputNumber placeholder="请输入" size="small" @on-focus="selectIndex = index" v-if="row.goodsName" :min="0"
  ...
  export default {
    ...
    data() {
      return {
        ...
        producerShow: false,
        goodsShow: false,
        allStockClass: ["借用出库", "领料出库", "零售出库", "盘点出库", "报损出库", "调拨出库", "网点发货出库", "加工成品出库", "委外出库", "返修出库", "批发出库",
          "归还出库", "其他出库"
        ],
        formItem: {
          stock: '',
