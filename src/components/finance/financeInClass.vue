	...
	<div style="position: relative;">
		...
		<div style="margin-bottom: 10px;">
			<Breadcrumb separator="<b style='color: #999;'>/</b>">
				...
				<BreadcrumbItem to="/home/goods">收入类别管理</BreadcrumbItem>
			</Breadcrumb>
		</div>
...
<script>
	...
	export default {
		...
		mounted() {
			that = this;
			finance.get_fristclass("financeIn").then(res => {
				if(res.length > 0){
		...
		methods: {
			...
			add_fristclass() {
				that.loading = true;
				finance.add_fristclass(that.frist_classtext,"financeIn").then(res => {
					finance.get_fristclass("financeIn").then(res => {
			...
			add_secondclass() {
				that.loading = true;
				finance.add_secondclass(that.select_item.objectId, that.second_classtext,"financeIn").then(res => {
					console.log(res)
					if (res) {
