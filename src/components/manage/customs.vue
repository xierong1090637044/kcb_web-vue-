...
<template>
	<div style="position: relative;">
		...
		<!--添加客户-->
		<Modal v-model="modal3" title="添加客户" @on-ok="add_custom">
			<Form :label-width="80">
				<FormItem label="名字">
...
<script>
	...
	export default {
		...
		methods: {
			...
			edit(row) {
				that.modal3 = true;
				that.custom.name = row.name
				that.custom.address = row.address
				that.custom.phone = row.phone
				that.custom.disabled = row.disabled
				that.custom.beizhu = row.beizhu
