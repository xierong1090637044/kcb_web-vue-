					...
					<Tabs v-model="loginType" @on-click="handleData">
						<TabPane label="账号密码登陆" icon="logo-apple" name="way1">
							<Form ref="formInline" :model="formInline" :rules="ruleInline">
								<Alert type="error" show-icon v-if='alter_type =="phone_landing_error"'>手机号不存在或者验证码不对</Alert>
								<Alert type="error" show-icon v-if='alter_type =="time_landing_error"'>本地电脑时间存在误差，请校正电脑时间，步骤查看小程序的操作手册</Alert>
								<FormItem prop="phone">
						...
						<TabPane label="手机验证码登陆" icon="logo-windows" name="way2">
							<Form ref="formInline" :model="formInline" :rules="ruleInline">
								<Alert type="error" show-icon v-if='alter_type =="phone_landing_error"'>手机号不存在或者验证码不对</Alert>
								<Alert type="error" show-icon v-if='alter_type =="time_landing_error"'>本地电脑时间存在误差，请校正电脑时间，步骤查看小程序的操作手册</Alert>
								<FormItem prop="phone">
									<Input type="text" v-model="formInline.phone" placeholder="请输入手机号" autofocus :maxlength="11">
									<Icon type="md-phone-portrait" slot="prepend" size="18" />
									</Input>
								</FormItem>
			...
			phone_login(phone, code) {
				...
				if (that.loginType == "way2") { //手机获取验证码登陆
					...
					Bmob.verifySmsCode(smsCode, data).then(function(response) {
						...
						that.button_login = false
						if (error.code == 400) {
							that.alter_type = "time_landing_error"
						} else {
