		...
		<Modal title="详情" v-model="detailShow" width="60%">
			...
			<div id="printTest" v-if="detail.type==3">
				<div>
					...
					<div class="display_flex_bet" style="margin-bottom: 10px;">
						<div style="font-size:16px;font-family:宋体;">盘点仓库：{{detail.stock?detail.stock.stock_name:''}}</div>
						<div style="font-size:16px;font-family:宋体;">盘点日期：{{detail.createdTime?detail.createdTime.iso.split(" ")[0]:detail.createdAt}}</div>
					</div>
				</div>
					...
					for (let item of res) {
						...
						if(item.type == 3){
							for(let detail of item.detail){
								console.log(detail.reserve)
								detail.reserve = Number(detail.reserve).toFixed(0)
							}
						}
